...
struct SrtEngine::SubtitleItem{
    // Structure for storing each subtitle, start, end stands for the starting and ending time in milliseconds, text stands for the content.
...
QString SrtEngine::currentSubtitle(long long time, bool sliderMoved){
    ...
    if(lastIndex != -1){
        ...
        for(int i = lastIndex, len = subtitles.size(); i < len; i++){
            ...
            if (time >= item.start && time <= item.end) return item.text;
        }
    }else{
