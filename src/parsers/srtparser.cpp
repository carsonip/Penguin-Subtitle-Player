...
std::vector<Engine::SubtitleItem> SrtParser::parseFile(QFile &f, QString encoding) {
    ...
    while (it.hasNext()) {
        ...
        long long start = Engine::calculateTime(m.captured(2), m.captured(3), m.captured(4), m.captured(5));
        long long end = Engine::calculateTime(m.captured(6), m.captured(7), m.captured(8), m.captured(9));
        QString text = m.captured(11);

        // Validate current subtitle section against previous:
        Engine::SubtitleItem currentItem = Engine::SubtitleItem(section,start,end,text);
